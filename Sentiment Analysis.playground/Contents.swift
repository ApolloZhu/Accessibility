...
do {
//: ## Importing Datasets
func dataTable(fromFile url: URL) throws -> MLDataTable {
...
let sentimentClassifier = try MLTextClassifier(
    trainingData: try dataTable(fromFile: #fileLiteral(resourceName: "training.1600000.processed.noemoticon.csv")),
    textColumn: "Text",
    labelColumn: "Label"
)
//: ... and evaluate it with the test dataset
let evaluationMetrics = sentimentClassifier.evaluation(on: try dataTable(fromFile: #fileLiteral(resourceName: "testdata.manual.2009.06.14.csv")))
//: ## Check Our Results
...
//: - Important: We should pay tribute to Sentiment140 as we used their dataset:
    additional: ["Bibliography":"A. Go, R. Bhayani, and L. Huang, “Twitter Sentiment Classification Using Distant Supervision,” in CS224N Project Report, 2009."]
)
//: Save the trained model as `Sentiment140.mlmodel`
let url = Bundle.main.resourceURL!.appendingPathComponent("Sentiment140.mlmodel")
try sentimentClassifier.write(to: url, metadata: metadata)
//: Lastly, open it in Finder so we can use it later!
NSWorkspace.shared.open(url)
//: Now we're all set. Have a nice day!
} catch {
    print(error)
}
